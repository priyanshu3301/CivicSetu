...
export default function ReportCard({ report }) {
    const { user } = useAuth();
    const [upvotes, setUpvotes] = useState(Array.isArray(report.upvotes) ? report.upvotes.length : 0);
    const [isUpvoted, setIsUpvoted] = useState(Array.isArray(report.upvotes) && user ? report.upvotes.includes(user._id) : false);
